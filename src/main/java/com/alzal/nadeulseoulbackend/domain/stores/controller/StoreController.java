...
public class StoreController {
    ...
    * */
    ...
    public ResponseEntity<Response> getStoreBookmarkList() {
        ...
        response.setData(storeService.getStoreBookmarkInfoDto(userSeq));
    ...
    public ResponseEntity<Response> insertStoreBookmark(@PathVariable("store_seq") Long storeSeq) {
        Long userSeq = 1L; //임시 : 사용자 토큰으로 가져와야함

        Response response = new Response();
        HttpHeaders httpHeaders = new HttpHeaders();
        storeService.insertStoreBookmark(userSeq, storeSeq);
        response.setStatus(StatusEnum.OK);
    ...
    public ResponseEntity<Response> getIsBookmark(@PathVariable("store_seq") Long storeSeq) {
        ...
        response.setStatus(StatusEnum.OK);
        response.setMessage("장소 찜하기 성공");
        boolean isBookmark = storeService.getIsBookmark(userSeq, storeSeq);
        response.setData(isBookmark);
        return new ResponseEntity<Response>(response, httpHeaders, HttpStatus.OK);
    }
